...
const { existenceUser } = require("../models/user");
const { existenceDevice } = require("../models/device");
const { existenceUserDevice } = require("../models/device");
const {
  existenceTicketAssign,
  setFalseActiveCloseTicket,
} = require("./ticket_assign");
...
async function createTicket(req, res) {
  ...
  if (!(await existenceUserDevice(sender_info.user_id, req.data.device_id))) {
    response_handler.sendFail(
      ...
      "this user is not the owner of this device!"
    );
  } else {
    let info = {
...
async function getTicketByAuthor(req, res) {
  if (!(await existenceUser(req.data.author))) {
    response_handler.sendFail(
      res,
      c.statusCodes.BAD_REQUEST,
...
async function getTicketByDate(req, res) {
  ...
  if (userAccess.isStaff) {
    tickets = await Ticket.getTicketByDate(
      req.data.start_date,
      req.data.end_date,
      "author",
      userAccess.user_id
...
async function setStatusTicketClose(req, res) {
  ...
  if (!(await existenceTicket(req.data.id))) {
    ...
    response_handler.sendFail(
      res,
      c.statusCodes.BAD_REQUEST,
      "ticket doesn't assign to this supporter"
    );
  } else {
