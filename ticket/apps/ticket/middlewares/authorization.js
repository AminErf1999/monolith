...
module.exports.isAdmin = (req, res, next) => {
  ...
  if (!token) {
    return sendFail(res, c.statusCodes.UNAUTHORIZED, {
      message: c.messages.invalidToken,
    });
  }
...
module.exports.isAdminSupporter = (req, res, next) => {
  ...
  if (payload.role === 1 || payload.role === 2) {
    ...
    return sendFail(res, c.statusCodes.UNAUTHORIZED, {
      message: "user is not admin or supporter",
    });
  }
};
...
module.exports.isStaffSupporter = (req, res, next) => {
  ...
  if (payload.role === 3 || payload.role === 2) {
    ...
    return sendFail(res, c.statusCodes.UNAUTHORIZED, {
      message: "user is not staff or supporter",
    });
  }
};
