...
async function login(req, res) {
    ...
    if (!(username && password)) {
        return response_handler.sendFail(res, c.statusCodes.UNAUTHORIZED, { message: c.messages.loginFailed })
    }
    const encryptedPassword = await bcrypt.hash(password, 10);
    if (username !== credentials.username || (await bcrypt.compare(encryptedPassword, credentials.password))) {
        return response_handler.sendFail(res, c.statusCodes.UNAUTHORIZED, { message: c.messages.loginFailed })
    }
