...
const TicketAssign = require('../../ticket/models/ticket-assign');
...
async function isTicketAuthorStaff(user_id, ticket_id) {
    let ticket = await Ticket.getTicketById(ticket_id);
...
async function isValidSender(payload, ticket_id) {
    if (payload.role === 2 && !await isTicketAssignedToSupporter(payload.user_id, ticket_id)) {
        return false;
    } else if (payload.role === 3 && !await isTicketAuthorStaff(payload.user_id, ticket_id)) {
        return false;
    }
    return true;
}
...
async function createMessage(req, res) {
    ...
    if (!await isValidSender(payload, req.data.ticket_id)) {
        response_handler.sendFail(res, c.statusCodes.BAD_REQUEST, "sender is not author/supporter of ticket");
    } else if (!await isTicketOpen(req.data.ticket_id)) {
        response_handler.sendFail(res, c.statusCodes.BAD_REQUEST, "ticket is closed.");
    } else {
        let info = {
        ...
        await message.save(info).then((result) => console.log('save message to db'));
